...
<template>
    <div>
        ...
        <v-layout row>
            ...
            <v-flex md5 lg5 class="ma-5">
                <v-select name="fanfiction" v-bind:items="fanfiction" v-model="favorite.fanfiction" label="Select Fanfiction" class="input-group--focused" item-value="fanfiction" :error-messages="errors.collect('fanfiction')" v-validate="'required'" data-vv-name="fanfiction" required></v-select>
            </v-flex>
            <v-flex md2 lg2 class="ma-5">
                <v-btn @click="addToFavorites">Submit</v-btn>
            </v-flex>
        </v-layout>
        <v-layout row>
